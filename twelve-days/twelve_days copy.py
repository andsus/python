...
def recite(start_verse, end_verse):
    ...
    for i in range(start_verse, end_verse+1):
        result = "On the {0} day of Christmas my true love gave to me: ".format(DAYS[i]) 
        zero_day = PHRASES[-1].format("" if i == 1 else "and " )
        result += "".join(PHRASES[12-i:-1]) + zero_day
    return [result]

def r(start_verse, end_verse):
